...
## Granting Users Access {#granting-users-access}

When using OIDC for authentication, administrators no longer need to create and manage users in NGINX Management Suite. Instead, they can create user groups in NGINX Management Suite that correspond to groups in their Identity Provider. The assigned roles for the user group determine the level of access and permissions for users based on their group membership. Users from the Identity Provider who do not belong to a group with an assigned role will not have access to NGINX Management Suite.

Learn how to grant users access using OIDC by following these steps:

1. Create a role,
1. Create a user group with an assigned role,
1. Ensure that there is a corresponding group in your identity provider,
1. Configure OIDC.
...
### Create a Matching Group in Your Identity Provider

To ensure proper integration between NGINX Management Suite and your Identity Provider (IdP), you must create a group in your IdP that exactly matches the group name you created in NGINX Management Suite. Additionally, the group claim must be included in the token generated by your IdP. For instructions on adding group claims, consult your IdP's documentation.


After you've successfully [created a user group](#create-user-group) in NGINX Management Suite and a corresponding group in your IdP, you can proceed with configuring OIDC.
...
3. If you have additional modules installed, such as API Connectivity Manager, you need to modify their specific configuration files as well.
   ...
   Open the API Connectivity Manager configuration file `/etc/nms/nginx/locations/nms-acm.conf` in an editor and uncomment the OIDC settings starting with `#OIDC`. Comment out the settings for Basic Authentication. Save the changes.
