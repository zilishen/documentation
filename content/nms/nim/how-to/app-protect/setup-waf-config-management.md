...
```bash
sudo apt-get install nms-nap-compiler-v4.762.0
```
...
```bash
sudo apt-get install nms-nap-compiler-v4.762.0 -o Dpkg::Options::="--force-overwrite"
```
...
sudo yum install nms-nap-compiler-v4.762.0
```
...
For example, to view the help description for WAF compiler 4.762.0, run the following command:

``` bash
sudo /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -h
```
...
```text
USAGE:
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile <options>

Examples:
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -p /path/to/policy.json -o mypolicy.tgz
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -p policyA.json -g myglobal.json -o /path/to/policyA_bundle.tgz
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -g myglobalsettings.json --global-state-outfile /path/to/myglobalstate.tgz
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -b /path/to/policy_bundle.tgz --dump
    /opt/nms-nap-compiler/app_protect-4.762.0/bin/apcompile -l logprofA.json -o /path/to/logprofA_bundle.tgz
...
```
