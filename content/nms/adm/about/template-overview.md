...
description: Template overview of the App Delivery Manager(ADM) Module.
weight: 300
toc: true
draft: false
tags: ["docs"]
...
{{< note >}}
- ADM templates only control the snippet of config generated by the ADM module. 
- The config generated by the ADM module is merged with the base config and config from other modules to generate the final config.
...
- In the final config, the order of main, http, stream, server, upstream, match, map, etc...directives will match what is in the template. But the line number of where the ADM config starts will be different and cannot be controlled via ADM templates.
{{< /note >}}
...
{{< warning >}}
- We recommend **not** updating the built-in templates, as these will potentially change from one release to the next. ADM will enforce this via MD5 checks built into the code.
{{< /warning >}}
### Gateway
...
## Expanding the ADM API using Use Case templates.
...
## Upgradability and supportability

ADM ships with built-in templates and use cases. Future ADM versions may change the base and use case templates. Modifying them is not supported and may cause upgrades to fail. ADM will also prevent these modifications via MD5 checks. The recommended ways of enhancing the templates are:
