...
With NGINX Plus, you can dynamically reconfigure [load balancing]({{< relref "admin-guide/load-balancer/dynamic-configuration-api.md" >}}) across the servers in an upstream group without reloading the configuration. You can also use the NGINX Plus API and key‑value store to dynamically control access, for example [based on client IP address]({{< relref "admin-guide/security-controls/denylisting-ip-addresses.md" >}}).
