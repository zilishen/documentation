...
Just like attack signatures, the Threat Campaign patterns, i.e. `app-protect-attack-signatures` and `app-protect-threat-campaigns` are installed as a dependency chain of our main package `app-protect`. Then, the `app-protect-attack-signatures` and `app-protect-threat-campaigns` should be updated periodically to get the latest security updates. Due to the highly dynamic nature of those campaigns the updates are issued far more frequently than the attack signatures. You need to install those updates close to the time they are issued in order to get the most effective protection.
