...
4. The **strict-post** rule is triggered when there are POST requests containing either the session token "c2Vzc2lvblRva2Vu" in the cookies or the "gzip" value in the content-encoding headers. This rule follows a similar approach to referencing an external policy file, just like the **api-strict** rule mentioned above.
