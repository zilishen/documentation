...
- The API Management module is available to users with the predefined [Admin or User Roles]({{< relref "/platform/access-management/manage-roles.md#predefined-roles-and-role-groups" >}}).
...
- [App]({{< relref "app-delivery/manage-apps.md" >}})

- [Identity Provider]({{< relref "services/manage-identity-providers.md" >}})

 (required to add Authentication to the Published API Component).

{{< tip >}}
You can connect one or more [Developer Portals]({{< relref "api-management/manage-dev-portals.md" >}}) to your Published API to host your API documentation. This can be done either when creating or editing your Published API, or independently via the API Quick Actions menu.
{{< /tip >}}
...
3. On the **Create App Component** *Workload Groups* page:
   ...
   - Refer to the [Manage Locations]({{< relref "infrastructure/locations/manage-locations.md" >}}) topic for more information.
...
5. On the **Create App Component** *Authentication* page:
    ...
    2. Select an [**Identity Provider**]({{< relref "services/manage-identity-providers.md" >}}).
...
6. On the **Create App Components** *Advanced Security* page:
    ...
    2. (Optional) Select **Monitor Only** to allow traffic to pass without being rejected. Security events are still generated and metrics are still collected. Refer to [About App Security Analytics]({{< relref "analytics/view-app-security-analytics.md" >}}) for more information.

    3. (Optional) Add the signature(s) that you want WAF to ignore. You can specify multiple signatures as a comma-separated list.

    4. Select **Next**

    {{< see-also >}} Refer to the [Default WAF Policy]({{< relref "app-delivery/security/concepts/app-sec-default-policy-original.md" >}}) topics to learn more about the default protection provided by NGINX App Protect. {{< /see-also >}} 
...
From the **API Definitions** page, select **Create Dev Portal** from the Quick Actions menu. Then, follow the steps in [Create a Developer Portal]({{< relref "api-management/manage-dev-portals.md" >}}) to create, customize, and publish your Dev Portal.
