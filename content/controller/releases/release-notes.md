...
- Improvements to the [Data Explorer]({{< relref "analytics/data-explorer/how-to-use.md" >}}) make it easier to see your data's dimensions and preview the discrete values.
...
- **Easily enable or disable High Availability mode by updating gateways**
  ...
  Prior to NGINX Controller v3.17, you had to delete and recreate gateways to [enable or disable HA (high availability) mode]({{< relref "infrastructure/instances/ha-data-plane.md" >}}) . Now, you can enable or disable HA mode on the fly by simply updating the existing gateway.
...
- **Association Token licenses no longer require port 8883**
  ...
  See [Licensing NGINX Controller]({{< relref "platform/licensing-controller.md" >}}) for instructions.

- **Adds support for SSL settings in health checks when multiple virtual hosts are defined per workload group member**

  Auto-generated health check location blocks now include any `proxy_ssl` settings that have been defined in the TLS object in the web backend object. For example, using the [NGINX Controller REST API]({{< relref "api/_index.md" >}}) to set
...
- **Active Directory integration adds support for LDAPS and unencrypted LDAP**
  ...
  Refer to [Configure Active Directory Integration]({{< relref "platform/access-management/manage-active-directory-auth-provider.md" >}}) for instructions on how to add an Active Directory provider to NGINX Controller for external authentication.

- **Data Forwarders are no longer restricted**

  While in beta, [Data Forwarders]({{< relref "analytics/forwarders" >}}) had restrictions to limit the number of stream sources that could be defined. These restrictions have been lifted.
...
- **Upgrading NGINX Controller Agent fails with error "can't find %s" (24019)**
  ...
  **Workaround:**
  ...
  To resolve this error, you'll need to uninstall and then re-install the Controller Agent. Refer to the [NGINX Controller Agent Installation Guide]({{< relref "admin-guides/install/install-nginx-controller-agent.md" >}}) for instructions.
...
- **Introduces NGINX Controller's own Dev Portal as the user interface for the NGINX Controller API Reference guide**
  ...
  [https://docs.nginx.com/nginx-controller/api/reference/ctlr-v1/]({{< relref "api/_index.md" >}})

  Refer to the [API Management documentation]({{< relref "/api-management" >}}) to learn more about using Dev Portals for your APIs.
...
- **Installing NGINX Controller on a misconfigured NFS server fails with the error "Bundled DB did not start successfully" (22751)**
  ...
  When installing NGINX Controller on an NFSv3 server, if the [NFS requirements]({{< relref "/admin-guides/install/nginx-controller-tech-specs.md#nginx-controller-database-requirements" >}}) are not met, the installation may fail with an error similar to the following: "Bundled DB did not start successfully."
...
- **Monitor your apps' metrics with Alert Rules**
  ...
  Now you can create [Alert Rules]({{< relref "/analytics/alerts/about-alerts#alert-rules" >}}) for app-centric metrics using the NGINX Controller web interface or [REST API]({{< relref "api/_index.md" >}}). Also, the web interface for Alerts has a new and improved design.

  To create Alert Rules, refer to the [Manage Alerts]({{< relref "/analytics/alerts/manage-alerts" >}}) topic. For information about the metrics that you can monitor, see the [NGINX Controller Metrics Catalog]({{< relref "/analytics/catalogs/metrics" >}}).
...
- **View the percentage of requests that WAF has blocked or flagged**
  ...
  Go to [View App Security Analytics]({{< relref "/app-delivery/security/tutorials/view-app-security-analytics" >}}) to get started.
...
- **Restoring NGINX Controller from a backup without restoring the config and encryption keys causes the system to become unresponsive (22066)**
  ...
  When restoring NGINX Controller from a backup, you also need to restore the NGINX config and encryption keys; otherwise, the system may become unresponsive. For instructions, refer to [Back Up & Restore Cluster Config and Encryption Keys]({{< relref "/admin-guides/backup-restore/backup-restore-cluster-config" >}}).
...
- **Steps to restore embedded config database from a specific backup file are incorrect (22044)**
  ...
  **Workaround:**
  ...
  To restore the embedded config database, follow the instructions in the [Backup & Restore Embedded Config Database]({{< relref "/admin-guides/backup-restore/backup-restore-embedded-config-db#restore-embedded-config-database" >}}) guide that's online.
...
- **Adds support for NGINX Streams (TCP & UDP)**
  ...
  To learn more, check out the [Manage Gateways]({{< relref "services/manage-gateways.md" >}}) and [About Application Delivery]({{< relref "/app-delivery/about-app-delivery.md" >}}) topics.
...
- **Improved experience deploying the NGINX Controller with App Security Add-on**
  ...
  To begin using NGINX Controller with the Application Security Add-on, follow the steps in the [Trial NGINX Controller with App Security]({{< relref "admin-guides/install/try-nginx-controller-app-sec.md" >}}) quick start guide.
...
- **Data forwarding for Splunk now includes NGINX Controller events**
  ...
  To get started, refer to [Forward Analytics Data to Splunk]({{< relref "/analytics/forwarders/forward-analytics-to-splunk.md" >}}).
...
- **Usage information is now reported with licensing information**
  ...
  For instructions on how to view and manage your licenses, see [License NGINX Controller]({{< relref "/platform/licensing-controller.md" >}}).
...
- **NGINX Controller deletes some alert rules on upgrade to v3.13 (20882)**
  ...
  - The [ServiceNow Alerts integration]({{< relref "/analytics/alerts/service-now-notifications.md" >}}) is no longer supported. Subscribers of any type other than `email` will be ignored. Alert rules which do not have at least one email subscriber will not be migrated.
...
- **Users with an environment-specific role can't create or update Components using the browser interface (22021)**
  ...
  **Workaround:**
  ...
  Use the `/platform/roles` endpoint in the [NGINX Controller REST API]({{< relref "api/_index.md" >}}) to add the following permissions to the custom role:
...
<br/><br/>
NGINX Controller requires an [8-core CPU @ 2.4 GHz or higher]({{< relref "/admin-guides/install/nginx-controller-tech-specs.md#hardware-specifications" >}}).
...
Refer to the [NGINX Controller Tech Specs]({{< relref "/admin-guides/install/nginx-controller-tech-specs" >}}) guide for hardware requirements and system recommendations.
...
- **Deploy NGINX Controller as a resilient cluster on a private cloud**
  ...
  Now you can [deploy NGINX Controller as a multi-node cluster on a private cloud]({{< relref "/admin-guides/install/resilient-cluster-private-cloud" >}}) to provide uptime resiliency. A multi-node cluster ensures that NGINX Controller stays up even if one of the control-plane hosts becomes unavailable.
...
- **Set up data plane high availability (HA)**
  ...
  [Configure a high-availability data plane for your apps]({{< relref "/infrastructure/instances/ha-data-plane" >}}) in on-premises deployments using NGINX Controller, NGINX Plus, and keepalived. High-availability data planes help to ensure your apps operate continuously without service interruptions.
...
Refer to the [NGINX Controller Tech Specs]({{< relref "/admin-guides/install/nginx-controller-tech-specs.md" >}}) guide for hardware requirements and system recommendations.
  {{< /caution >}}
...
- **To view Identity Providers and API Definitions, new users need at least `guest` role (20182)**
  ...
  **Workaround:**
  ...
  When creating a new user, you must assign the `guest` role (or higher) for the user to view Identity Providers and API Definitions. Alternatively, if you're using a custom role, you can use the API to grant explicit permissions for the `/security/identity-providers` and `/services/api-definitions` endpoints. See the [Manage Roles and Role Groups]({{< relref "/platform/access-management/manage-roles" >}}) topic details.
...
- **Floating IP has to be removed to be updated (19011)**
  ...
  1. To remove the existing floating IP using the [NGINX Controller REST API]({{< relref "api/_index.md" >}}), send a PATCH request to the `/platform/global` endpoint. Submit an empty string for the `clusterFloatingIP` value.
...
- **API Reference does not specify that TCP/UDP App Components are experimental (20285)**
  ...
  In the [NGINX Controller API Reference]({{< relref "api/_index.md" >}}), the schema for App Components includes two objects related to TCP/UDP support that are missing the **x-f5-experimental** extension: `ComponentTcpUdpDesiredState` and `ComponentTcpUdpCurrentState`. TCP/UDP App Components are not supported in NGINX Controller v3.12.
...
- **System Properties dialog showing current and available Agent versions was removed (17998)**
  ...
  **Workaround:**
  ...
  When you upgrade NGINX Controller, you must upgrade the Controller Agent as well to keep the versions in sync, following the instructions here:  [Controller Agent Installation Guide]({{< relref "/admin-guides/install/install-nginx-controller-agent.md" >}}).
